                ...
                <ScrollView style={{ flex: 1, marginBottom: '15%' }} >
                    {
                        this.state.data.length !=0 ? (this.state.data.map((item, index) => {
                            var roomID = `${this.state.doc_id}-${item.id}`
                            console.log("*********ROOM ID",roomID)
                            return (

                                <TouchableOpacity key={index} onPress={() => this.props.navigation.navigate("DocPatAppoint" , { patient: item } )} style={styles.typeView}>
                        ...
                        })) : (<TextCommon  text="Nessuna videochiamata effettuata" color={"#000000"} fontSize={global.fontSize_15} fontWeight={"600"} marginTop={"10%"} textAlign={"center"} fontFamily={"Montserrat-Bold"} />)
                    }
                </ScrollView>
