...
export default class secondScreen extends Component {
    ...
    onPressConfirm = async () => {
        const jsonUserResponse = await this.getUser_Detail()
            .catch(error => console.error('getUserDetail', error));
        const userResponse = JSON.parse(jsonUserResponse);
        ...
        if (this.state.from === 'current') {
            ...
            this.handlePastPathology(userResponse.patient, pathologyResponse.pathology)
    ...
    handleCurrentPathology = async (pathology) => {
        ...
        this.props.navigation.navigate("thirdScreen", {pathology, current: null, from: this.state.from});
    }
    ...
    rowClicked(pathology) {
        if (this.state.from === "current") {
            this.props.navigation.navigate("thirdScreen", {pathology: null, current: pathology, from: this.state.from});
        }
    }

    getAllPathologiesApiCall = async () => {
        const user = JSON.parse(await this.getUser_Detail());
        ...
        if (this.state.from === "current") {
            const currentPathologiesResponse = await getCurrentPathologyByPatient({patient_id: user.patient.id});
            currentPathologies = currentPathologiesResponse.currentPathologies;
            results = currentPathologies;
        } else {
            const pastPathologiesResponse = await getPastPathologyByPatient({patient_id: user.patient.id});
            ...
            <SafeAreaView style={{
                ...
                <Modal style={{backgroundColor: 'rgba(0, 0, 0, 0.5)'}} animationType="slide" transparent={true} visible={this.state.addNewPathologyModal}>
                    <View style={{
                        ...
                        >
                            <Text style={{ textAlign: 'justify', color: global.primary, fontSize: 19}}>
                ...
                <CustomeHeader backgroundColor={global.primary}
                ...
                <View style={{
                    ...
                    <TouchableOpacity
                        onPress={() => {
                            this.setState({
                                addNewPathologyModal: !this.state.addNewPathologyModal,
                            });
                        }}
                        style={{
                            margin: 3,
                    ...
                    >
                        <Text style={{flex: 1, color: "#FFF", textAlign: "center", fontSize: global.fontSize_14}}>Aggiungi
                            patologia</Text>
                    </TouchableOpacity>
                </View>
                <ScrollView style={{flex: 1}}>
                    {Object.entries(this.state.data).filter((entry) => entry[1].name.includes(this.state.searchTxt)).length !== 0 ? (
                        Object.entries(this.state.data).filter((entry) => entry[1].name.includes(this.state.searchTxt))
                            .map((value) => {
                                return (
                        ...
                        <TextCommon
                            text={this.state.searchTxt.length === 0 ? `Non hai ancora inserito patologie ${this.state.from === "current" ? "in corso" : "passate"}, aggiungile.` : `Non hai inserito patologie ${this.state.from === "current" ? "in corso" : "passate"} che corrispondano al filtro inserito.`}
